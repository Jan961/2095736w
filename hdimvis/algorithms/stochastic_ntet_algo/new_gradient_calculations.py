...
# returns an array of shape (n-tet size, 2)
...
def compute_quartet_grads(points : np.ndarray, Dhd : np.ndarray, Dld : np.ndarray,
    ...
    while row < diffs.shape[0] - 1:
        ...
        for i in range(2):
            ...
            temp_points1 = np.expand_dims(temp_points, axis=1) # using this instead of [:,None] to help Numba
            temp_points2 = np.expand_dims(temp_points, axis=1).T
